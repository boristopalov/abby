    ...
    function selectTrack(track: Track) {
        ...
        if (hashIndex !== -1) {
            ...
            const endIndex =
                nextSpaceIndex === -1
                    ? chatInput.length
                    : hashIndex + nextSpaceIndex;
            chatInput =
                chatInput.slice(0, hashIndex) + chatInput.slice(endIndex);
        }
    }
    ...
    <div class="messages-outer" bind:this={messagesContainer}>
        <div class="messages-column">
            {#each messages as message}
                <div
                    class="message-row {message.isUser
                        ? 'message-row--user'
                ...
                >
                            ...
                            <div class="tool-header">
                                ...
                                {#if message.arguments && Object.keys(message.arguments).length > 0}
                                    <span class="tool-args">
                                        ({Object.entries(message.arguments)
                                            .map(
                                                ([k, v]) =>
                                                    `${k}=${JSON.stringify(v)}`,
                                            )
                    ...
                    {:else if message.type === "approval_required"}
                        ...
                        <div class="msg-approval">
                            ...
                            {:else}
                                <div
                                    class="approval-decided approval-decided--{message.approvalState}"
                                >
...
<style>
    ...
    .msg-user {
        ...
        border-radius: var(--radius-lg) var(--radius-lg) var(--radius)
            var(--radius-lg);
